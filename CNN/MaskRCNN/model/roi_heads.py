...
from .pooler import RoIAlign, roi_align
from .utils import Matcher, BalancedPositiveNegativeSampler
from .utils import BoxCoder, box_iou, process_box, nms
...
def maskrcnn_loss(mask_logit, proposal, matched_idx, label, gt_mask):
    ...
    mask_target = roi_align(gt_mask, roi, 1., M, M, sampling_ratio=-1)[:, 0]
...
class RoIHeads(nn.Module):
    ...
    def select_training_samples(self, proposal, target):
        ...
        Returns:
            proposal (Tensor[K, 4])
            matched_idx (Tensor[K])
            label (Tensor[K])
            regression_target (Tensor[K, 4])
        """
        gt_box = target['boxes']
